...
class Turtle:
    ...
    def type(self):
        self.penup()
        blockType = self.mc.getBlock(self.position.x, self.position.y-2, self.position.z)
        # self.mc.postToChat(
        #     f"{blockType}: { self.position.x, self.position.y-2, self.position.z}"
        # )
        self.pendown()
        return blockType
